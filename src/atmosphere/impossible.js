// Невозможные фигуры: Пенроуза, лестница Эшера, невозможная вилка.
//
// Это не формы и в каталог форм не идут: невозможная фигура существует ровно в одной
// проекции. Строится открытая фигура из брусков — честная, разомкнутая в пространстве —
// а её последний конец сдвигается ВДОЛЬ ЛУЧА ЗРЕНИЯ из точки привязки, пока не наложится
// в проекции на первый конец: P' = anchor + |P - anchor| * normalize(target - anchor).
// Из точки привязки фигура читается замкнутой; при отходе в сторону наложение расходится.
//
// Пары концов, которые обязаны совпасть в проекции, модуль отдаёт в `seams`: без них
// иллюзию нельзя проверить — разница между треугольником Пенроуза и кривой рамкой —
// один-два пикселя. Концы шва лежат на нарисованной фигуре.
...
// Три бруска по взаимно перпендикулярным осям: треугольник Пенроуза.
...
// Четыре марша со ступенями, каждый поднимается: лестница Эшера.
...
// Невозможная вилка: два зуба, каждый уходит вперёд по z, а третий — поперёк по x.
...
function beamsImpossibleFork(anchor, center, L) {
  ...
  const B = add(center, [-t, 0, z1]);
  const C = add(center, [t, 0, z0]);
  const D = add(center, [t, 0, z1]);
  const E = add(center, [0, 0, z0]);
  const naturalEnd = add(E, [2 * t, 0, 0]);
  const F = slideOntoRay(anchor, naturalEnd, A);
  return {
    beams: [[A, B], [C, D], [E, F]],
    seams: [{ a: A, b: F }],
  };
...
const KINDS = {
  ...
  impossibleFork: beamsImpossibleFork,
};
