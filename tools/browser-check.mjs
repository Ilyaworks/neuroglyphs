...
try {
  ...
  // Пересвет: свечение поверх плотного скопления глифов срезает все детали, и кадр
  // становится белым пятном. Замер по восьми мирам после R19: 0.18, 0.53, 0.56, 1.30,
  // 1.33, 2.53, 3.74, 4.43 процента — худший вдвое с лишним ниже порога. До R19 тот же
  // замер давал 12.41% по кадру и 74% по яркой четверти, то есть порог ловит возврат
  // к прежнему поведению, а не придирается к яркому миру.
  const MAX_CLIPPED = 0.10;
  if (expectContent && stats) {
    if (stats.clippedShare > MAX_CLIPPED) {
      problems.push('кадр выжжен: в чистый белый упирается ' +
        (stats.clippedShare * 100).toFixed(2) + '% пикселей при пороге ' +
        (MAX_CLIPPED * 100) + '%. В плотных местах глифы сливаются в пятно без деталей — ' +
        'смотри силу и порог свечения в src/render/post.js.');
    }
