...
const PROBE = [
  ...
  '    return Math.hypot(pa.x - pb.x, pa.y - pb.y) / Math.hypot(w, h);',
  '  });',
...
console.log('точек перед камерой: ' + (st.впереди * 100).toFixed(1) + '% (нужно не меньше ' +
  (MIN_AHEAD * 100) + '%)');
console.log('швы в проекции из настоящей камеры: ' +
  (st.швы.length ? st.швы.map(v => v.toFixed(5)).join(', ') : 'швов в метке нет') +
  ' (нужно ≤ ' + SEAM_MAX + ')');
...
if (!(screen >= MIN_SCREEN)) {
  bad('фигура занимает ' + screen.toFixed(3) + ' высоты кадра при пороге ' + MIN_SCREEN +
    ...
    'четырнадцати тысяч глифов игрок не найдёт, и признак 12 не выполнится.');
}
