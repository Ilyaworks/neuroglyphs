...
// Масса здания. Проверка «модуль вернул пять коробок» её не стережёт: её проходят и
// пять коробок рядом, и пять коробок в одном параллелепипеде, и арка, упирающаяся в
// стену. Самопроверка: node tools/massing-check.mjs --self
if (fs.existsSync('src/world/massing.js')) {
  const r = run('node tools/massing-check.mjs');
  if (!r.ok) refuse('масса здания не проходит проверку', r.out);
  console.log('масса в порядке: части срослись, силуэт рваный, арки сквозные');
}

// План города. Проверка «участков десять» ничего не стережёт: десять участков можно
// выстроить цепочкой, налепить друг на друга или связать щелями в полроста.
// Самопроверка: node tools/city-check.mjs --self
if (fs.existsSync('src/world/city.js')) {
  const r = run('node tools/city-check.mjs');
  if (!r.ok) refuse('план города не проходит проверку', r.out);
  console.log('город в порядке: сеть, а не цепочка; два пути к выходу; проходы настоящие');
}
