...
function healthCheck() {
  ...
  if (fs.existsSync('src/world/shapeStrange.js')) {
    checks.push(['причудливые формы', 'node tools/strange-check.mjs']);
  }
  if (fs.existsSync('src/world/locations')) {
    checks.push(['локации', 'node tools/location-check.mjs']);
  }
  ...
  for (const [label, cmd] of checks) {
    ...
    if (!r.ok) {
      ...
      console.log('');
      // Раньше здесь стояло «поломку принесла одна из предыдущих задач» — без всякой
      // проверки. На N31 это стоило человеку целого захода: он собрался откатывать
      // закрытую N30, у которой в коммите лежал только модуль, а надпись с нулём
      // пикселей жила в НЕЗАКОММИЧЕННОМ boot.js этой же сессии. Прежде чем указывать
      // на прошлое, смотрим на рабочее дерево.
      const dirty = tryRun('git status --porcelain');
      const changed = (dirty.out || '').trim().split(NL).filter(l => l.trim() && !/\.planning\//.test(l));
      if (changed.length) {
        console.log('В рабочем дереве есть НЕЗАКОММИЧЕННЫЕ правки кода:');
        console.log(changed.slice(0, 20).join(NL));
        console.log('');
        console.log('Значит, поломка почти наверняка в них, а не в прошлых задачах.');
        console.log('Откат закрытой задачи (undo-task) тут НЕ поможет: он отменит чужой');
        console.log('коммит и не тронет эти строки. Разбирайся с ними.');
        console.log('Модели: ничего не делай, напечатай STOP и покажи человеку этот список.');
        process.exit(2);
      }
      console.log('Рабочее дерево чистое, значит поломку принесла одна из закрытых задач.');
      console.log('Проверь по git log, какая задача трогала сломанный файл, и назови её');
      console.log('человеку по имени — не «последнюю закрытую» наугад:');
      console.log('  node tools/undo-task.mjs <ID той задачи, чей коммит трогал этот файл>');
      console.log('Человеку: если модель не справится, она отложит задачу через blocked.mjs.');
...
console.log(head);
console.log('');
console.log('## Как закончить задачу');
console.log('');
...
console.log('2. Не прошла после трёх честных попыток — НЕ вызывай finish-task и не начинай');
console.log('   заново. Отложи задачу и иди дальше:');
console.log('      node tools/blocked.mjs ' + task.id + ' "почему застряла, подробно"');
console.log('   Приложи человеку точный текст ошибки, затем выполни next-task.');
console.log('3. Прошла — выполни:');
console.log('      node tools/finish-task.mjs ' + task.id);
console.log('');
console.log('   Дальше по его ответу:');
console.log('');
console.log('   * ЗАКРЫЛАСЬ — выполни node tools/session-health.mjs. Скажет «можно');
console.log('     продолжать» — сразу next-task и следующая задача, человека не спрашивай.');
console.log('     Скажет «пора в новую сессию» — напечатай человеку то, что он велел,');
console.log('     и остановись.');
console.log('');
console.log('   * ОТКАЗАЛ словами «нет вердикта приёмки глазами» — это не поломка.');
console.log('     Пересними картинку, напечатай человеку блок «ЧТО ПРОВЕРИТЬ ГЛАЗАМИ»');
console.log('     ЦЕЛИКОМ вместе с готовыми фразами ответа, спроси «принято или не');
console.log('     принято?» и ЖДИ. Вердикт сам не выноси, другую задачу не бери.');
console.log('     Ответил «принято» — node tools/verdict.mjs ' + task.id + ' да --via-model,');
console.log('     потом finish-task снова. Ответил «не принято» — запиши его причину');
console.log('     через нет "..." --via-model и правь ровно названное.');
console.log('');
