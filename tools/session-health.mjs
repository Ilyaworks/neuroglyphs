...
if (msgs >= STOP_MSGS || tin >= STOP_TOK) {
  ...
  console.log('Скажи человеку ровно это и остановись:');
  console.log('');
  console.log('  Сессия выросла (' + msgs + ' сообщений, ' + tin + ' входных токенов).');
  console.log('  Дальше в ней работать не стоит: контекст сжимается и подробности');
  console.log('  теряются. Начните новую сессию — вставьте промт из .planning\\START.md');
  console.log('');
  console.log('Задачу при этом не бросай недоделанной: доведи текущую до закрытия');
  console.log('или до вопроса человеку, и только потом проси новую сессию.');
  process.exit(2);
}
